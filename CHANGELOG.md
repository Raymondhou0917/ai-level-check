...
- **報告漏人物志、重跑卻變成 Demo。** 貼上模式先前會去抓 README，模型跟著打開虛構示範 HTML，把林可安／周子寧／何柏廷抄成「你的報告」；骨架裡的人物志又只有一行代號，四軸格容易被省掉。現在：版型只准用 `report-skeleton.html`、明文禁止抓 Demo；總覽沒有代號＋四軸 2×2 不算交卷；重跑必須用同一批證據補區塊。
...
- **貼上模式 Prompt**：帶公開 repo 網址；禁止停在「請再貼紀錄／我沒有記憶」。有這一串就寫，沒有仍交 HTML。
